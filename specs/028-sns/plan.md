# Implementation Plan: Scaleway SNS (Topics & Events) MCP Tools

**Branch**: `028-sns` | **Date**: 2026-03-11 | **Spec**: specs/028-sns/spec.md

## Summary

Implement 8 MCP tools for the Scaleway Topics and Events (SNS) API covering service activation/deactivation, service info retrieval, and credentials CRUD. All tools are regional and use the shared Scaleway client infrastructure with the `@scaleway/sdk-mnq` package.

## Technical Context

**Language/Version**: TypeScript 5.x (strict mode)
**Primary Dependencies**: @modelcontextprotocol/sdk ^1.25.x, @scaleway/sdk-client ^1.0.0, @scaleway/sdk-mnq, zod ^3.25.x
...
```text
src/tools/sns/
├── index.ts          # Tool registration (registerSnsTools)
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/sns/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.

## Complexity Tracking

No complexity violations. Direct API mapping with shared infrastructure. The SNS API surface is small (8 tools) with a straightforward activate/deactivate + credentials CRUD pattern.
