# Implementation Plan: Scaleway Managed Inference MCP Tools

**Branch**: `029-inference` | **Date**: 2026-03-11 | **Spec**: specs/029-inference/spec.md

## Summary

Implement 15 MCP tools for the Scaleway Managed Inference API covering deployment CRUD, deployment events, endpoint management, model discovery, node type listing, and EULA management. All tools are regional (per Scaleway region) and use the shared Scaleway client infrastructure.
...
```text
src/tools/inference/
├── index.ts          # Tool registration (registerInferenceTools)
├── types.ts          # Zod schemas for all tool inputs and entity definitions
└── handlers.ts       # Tool handler implementations

tests/
├── unit/tools/inference/
│   └── handlers.test.ts    # Unit tests with mocked SDK
└── contract/tools/inference/
    └── contract.test.ts    # Contract tests validating API shapes
```

**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/inference/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.

## Complexity Tracking

No complexity violations. Direct API mapping with shared infrastructure. The `apiCall` helper in `handlers.ts` centralizes HTTP request logic for all 15 tools, keeping each handler minimal.
