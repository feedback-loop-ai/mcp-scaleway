# Implementation Plan: Scaleway Load Balancer MCP Tools

**Branch**: `017-lb` | **Date**: 2026-03-11 | **Spec**: specs/017-lb/spec.md

## Summary

Implement 28 MCP tools for the Scaleway Load Balancer API covering LB CRUD, migration, frontend management, backend management (including server pool operations), route management, certificate management, statistics, and type listing. All tools are zoned (per availability zone) and use the shared Scaleway client infrastructure.
...
```text
src/tools/lb/
├── index.ts          # Tool registration (registerLbTools)
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/lb/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.

## Complexity Tracking

No complexity violations. Direct API mapping with shared infrastructure. The backend creation handler has the most parameters (health checks, timeouts, proxy protocol) but follows the same pattern as all other handlers.
