# Implementation Plan: Scaleway NATS Messaging MCP Tools

**Branch**: `026-nats` | **Date**: 2026-03-11 | **Spec**: specs/026-nats/spec.md

## Summary

Implement 9 MCP tools for the Scaleway NATS Messaging API (MNQ v1beta1) covering NATS account CRUD (list, get, create, update, delete) and NATS credentials management (list, get, create, delete). All tools are regional and use the shared Scaleway client infrastructure.
...
```text
src/tools/nats/
├── index.ts          # Tool registration (registerNatsTools)
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/nats/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
