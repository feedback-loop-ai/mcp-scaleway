# Implementation Plan: Scaleway Managed Redis MCP Tools

**Branch**: `014-redis` | **Date**: 2026-03-11 | **Spec**: specs/014-redis/spec.md

## Summary

Implement 16 MCP tools for the Scaleway Managed Redis API covering cluster CRUD, cluster metrics and certificates, ACL rule management, endpoint management, and discovery (node types and versions). All tools are regional and use the shared Scaleway client infrastructure.
...
```text
src/tools/redis/
├── index.ts          # Tool registration (registerRedisTools)
...
└── contract/tools/redis/
    └── contract.test.ts    # Contract tests validating API shapes
```

**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/redis/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
