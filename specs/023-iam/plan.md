# Implementation Plan: Scaleway IAM MCP Tools

**Branch**: `023-iam` | **Date**: 2026-03-11 | **Spec**: specs/023-iam/spec.md

## Summary

Implement 28 MCP tools for the Scaleway IAM API covering user management, application management, API key management, policy and rule management, group management (including membership), and permission set discovery. The IAM API is global (not zoned) and uses the v1alpha1 API version.
...
```text
src/tools/iam/
├── index.ts          # Tool registration (registerIamTools)
├── types.ts          # Zod schemas for all tool inputs + entity schemas
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/iam/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.

## Complexity Tracking

No complexity violations. Direct API mapping with shared infrastructure. The IAM API is global (no zone routing), which simplifies implementation compared to zoned APIs.
