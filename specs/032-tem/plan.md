# Implementation Plan: Scaleway Transactional Email (TEM) MCP Tools

**Branch**: `032-tem` | **Date**: 2026-03-11 | **Spec**: specs/032-tem/spec.md

## Summary

Implement 15 MCP tools for the Scaleway Transactional Email API covering domain management, email sending/management, statistics retrieval, and webhook management. All tools are regional (per Scaleway region) and use the shared Scaleway client infrastructure.
...
```text
src/tools/tem/
├── index.ts          # Tool registration (registerTemTools)
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/tem/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
