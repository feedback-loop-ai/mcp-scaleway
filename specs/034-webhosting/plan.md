# Implementation Plan: Scaleway Web Hosting MCP Tools

**Branch**: `034-webhosting` | **Date**: 2026-03-11 | **Spec**: specs/034-webhosting/spec.md

## Summary

Implement 9 MCP tools for the Scaleway Web Hosting API covering hosting CRUD, hosting lifecycle (restore), DNS records, offers, and control panels. All tools are region-scoped and use the shared Scaleway client infrastructure.
...
```text
src/tools/webhosting/
├── index.ts          # Tool registration (registerWebhostingTools)
├── types.ts          # Zod schemas for all tool inputs and entities
└── handlers.ts       # Tool handler implementations

tests/
├── unit/tools/webhosting/
│   └── handlers.test.ts    # Unit tests with mocked SDK
└── contract/tools/webhosting/
    └── contract.test.ts    # Contract tests validating API shapes
```

**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/webhosting/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
