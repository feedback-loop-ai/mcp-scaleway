# Implementation Plan: Scaleway Instances MCP Tools

**Branch**: `002-instances` | **Date**: 2026-03-11 | **Spec**: specs/002-instances/spec.md

## Summary

Implement 19 MCP tools for the Scaleway Instances API covering server CRUD, server actions, volume management, security group management, IP management, and snapshot management. All tools are zoned (per availability zone) and use the shared Scaleway client infrastructure.
...
```text
src/tools/instances/
├── index.ts          # Tool registration (registerInstancesTools)
├── types.ts          # Zod schemas for all tool inputs
└── handlers.ts       # Tool handler implementations

tests/
├── unit/tools/instances/
│   └── handlers.test.ts    # Unit tests with mocked SDK
└── contract/tools/instances/
    └── contract.test.ts    # Contract tests validating API shapes
```

**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/instances/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
