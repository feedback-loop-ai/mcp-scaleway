# Implementation Plan: Scaleway Managed Database (RDB) MCP Tools

**Branch**: `013-rdb` | **Date**: 2026-03-11 | **Spec**: specs/013-rdb/spec.md

## Summary

Implement 26 MCP tools for the Scaleway Managed Database (RDB) API covering instance CRUD/upgrade, database management, user management, backup/restore, endpoint management, ACL rules, snapshots, and reference data. All tools are regional and use the shared Scaleway client infrastructure.
...
```text
src/tools/rdb/
├── index.ts          # Tool registration (registerRdbTools)
...
**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/rdb/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.
