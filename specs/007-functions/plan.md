# Implementation Plan: Scaleway Serverless Functions MCP Tools

**Branch**: `007-functions` | **Date**: 2026-03-11 | **Spec**: specs/007-functions/spec.md

## Summary

Implement 19 MCP tools for the Scaleway Serverless Functions API covering namespace CRUD, function CRUD + deploy, cron trigger management, custom domain management, and access token management. All tools are regional (fr-par, nl-ams, pl-waw) and use the shared Scaleway client infrastructure.
...
**Constraints**: Stateless proxy, all state in Scaleway API. API is v1beta1.
...
```text
src/tools/functions/
├── index.ts          # Tool registration (registerFunctionsTools)
├── types.ts          # Zod schemas for all tool inputs
└── handlers.ts       # Tool handler implementations

tests/
├── unit/tools/functions/
│   └── handlers.test.ts    # Unit tests with mocked SDK
└── contract/tools/functions/
    └── contract.test.ts    # Contract tests validating API shapes
```

**Structure Decision**: Single project structure. Tools are organized per product under `src/tools/functions/`. Tests mirror the structure under `tests/unit/` and `tests/contract/`.

## Complexity Tracking

No complexity violations. Direct API mapping with shared infrastructure. The `deploy` action is a simple POST with an empty body, not a multi-step workflow.
